...
class FarmIrisMod(loader.Module):
	...
	strings = {
		'name': 'farmiris',
		'farm_no_entity': '<i>Беседа с ирисботом не найдена, запусти его и попробуй снова:</i> <b>@iris_cm_bot</b>',
		'farm_not_4_bot': '<i>Данный модуль не предназначен для аккаунтов ботов</i>',
	...
	async def farmoncmd(self, message):
		...
		if self.is_bot:
			return await utils.answer(message, self.strings['farm_not_4_bot'])
		status = self.db.get(self.name, "status", False)
		if status: return await utils.answer(message, self.strings['farmon_already'])
		self.db.set(self.name, "status", True)
		try:
			await self.client.send_message(self.iris, "Фарма", schedule=timedelta(seconds=20))
		except:
			return await utils.answer(message, self.strings["farm_no_entity"])
		await utils.answer(message, self.strings['farmon'])
	...
	async def farmoffcmd(self, message):
		...
		await utils.answer(message, self.strings['farmoff'].replace("%coins%", str(coins)))
